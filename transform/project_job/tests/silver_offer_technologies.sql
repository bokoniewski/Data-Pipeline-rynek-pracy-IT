-- tests/silver_offer_technologies.sql
-- Testy dla modelu silver_offer_technologies
-- Zakres: tylko dane z bieżącego dnia (FILE_DATE)

-- not_null: id
SELECT 'not_null_id' AS test_name, id
FROM {{ ref('silver_offer_technologies') }}
...
-- not_null: technology_name
SELECT 'not_null_technology_name' AS test_name, id
FROM {{ ref('silver_offer_technologies') }}
WHERE {{ file_date_filter_by_offer_id() }}
AND technology_name IS NULL
