"""
=============================================================================
LOADER: load_raw_protocol.py
PROJEKT: Project_job
WARSTWA: Bronze
ŹRÓDŁO:  theprotocol.it – pliki JSON ze scrapera
=============================================================================
"""
...
def map_record(record: dict, file_id: int) -> tuple:
    ...
    offer_company zostawiony surowy — czyszczenie w silver.
    Wszystkie pola wielowartościowe serializowane do JSONB.
    """
    return (
        file_id,
        str(record.get("id", "")),
        record.get("url"),
        record.get("offer_title"),
        record.get("offer_company"),
        record.get("offer_level"),
        record.get("offer_modes"),
        record.get("offer_location"),
        json.dumps(record.get("salary_info"),            ensure_ascii=False),
        json.dumps(record.get("tech_required"),          ensure_ascii=False),
        json.dumps(record.get("tech_optional"),          ensure_ascii=False),
        json.dumps(record.get("offer_responsibilities"), ensure_ascii=False),
        json.dumps(record.get("offer_requirements"),     ensure_ascii=False),
        json.dumps(record.get("offer_nice_to_have"),     ensure_ascii=False),
        json.dumps(record.get("offer_offered"),          ensure_ascii=False),
        json.dumps(record.get("offer_benefits"),         ensure_ascii=False),
    )
...
def load_offers_to_bronze(file_path: str) -> LoadStats:
    """
    Główna funkcja ładująca oferty z pliku JSON do bronze.raw_offers_protocol.
    """
    source_file = Path(file_path).name

    if not source_file.startswith("protocol_"):
        raise ValueError(f"Nieprawidłowy plik: '{source_file}'. Loader obsługuje tylko pliki 'protocol_*.json'")
    ...
    try:
        ...
        cursor.execute(
            """
            SELECT offer_id FROM bronze.raw_offers_protocol
        ...
        insert_sql = """
            INSERT INTO bronze.raw_offers_protocol (
                source_file_id, offer_id, url,
                offer_title, offer_company, offer_level, offer_modes, offer_location,
                salary_info, tech_required, tech_optional,
                offer_responsibilities, offer_requirements,
                offer_nice_to_have, offer_offered, offer_benefits
            ) VALUES %s
        """
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Loader: bronze.raw_offers_protocol")
